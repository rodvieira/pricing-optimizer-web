  ...
  return (
    <div className="flex flex-col gap-3">
      <form onSubmit={onSubmit} className="flex flex-col gap-2">
        ...
        >
          <span aria-hidden className="pl-3 font-mono text-[14px] text-(--po-text-muted)">
            https://
          </span>
          <Controller
            control={control}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="font-sans text-[14px] text-(--po-text-muted)">{t("tryLabel")}</span>
        {EXAMPLE_URLS.map((example) => (
          <button
            ...
            className="rounded-[6px] border border-border bg-surface px-2.5 py-1 font-mono text-[14px] text-secondary transition-colors hover:border-border-strong disabled:opacity-60"
          >
            {example}
          </button>
        ))}
